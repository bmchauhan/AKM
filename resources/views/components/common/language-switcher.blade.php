...
<div class="relative z-[60] inline-block text-left" x-data="{ open: false }">
    @if ($variant === 'compact')
        <button
            type="button"
            @click="open = !open"
            class="inline-flex h-8 items-center gap-1 rounded px-1.5 text-xs font-semibold uppercase tracking-wide text-[#E6EBF4]/90 transition hover:bg-[#E6EBF4]/10 hover:text-[#E6EBF4]"
    ...
    >
        <div class="py-0.5">
            <a
                ...
                class="block px-4 py-2 text-sm text-[#0F141E] hover:bg-[#E6EBF4] {{ $locale === 'en' ? 'bg-[#ECEAE1] font-bold text-[#080D21]' : '' }}"
            >
                {{ __('messages.language_english') }}
            </a>
            <a
                href="{{ route('lang.switch', 'hi') }}"
                class="block px-4 py-2 text-sm text-[#0F141E] hover:bg-[#E6EBF4] {{ $locale === 'hi' ? 'bg-[#ECEAE1] font-bold text-[#080D21]' : '' }}"
            >
                {{ __('messages.language_hindi') }}
            </a>
            <a
                href="{{ route('lang.switch', 'gu') }}"
                class="block px-4 py-2 text-sm text-[#0F141E] hover:bg-[#E6EBF4] {{ $locale === 'gu' ? 'bg-[#ECEAE1] font-bold text-[#080D21]' : '' }}"
            >
