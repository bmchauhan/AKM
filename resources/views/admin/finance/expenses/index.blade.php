...
<x-layouts.admin :pageTitle="__('messages.finance_expenses')">
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-wider text-[#AB1E23]">{{ __('messages.finance') }}</p>
                <h2 class="mt-1 text-xl font-bold text-[#080D21]">{{ __('messages.finance_expenses') }}</h2>
                <p class="mt-1 text-sm text-[#0F141E]/70">{{ __('messages.finance_expenses_subtitle') }}</p>
            </div>

            <div class="flex flex-wrap gap-2">
                <x-common.button variant="secondary" :href="route('admin.finance.expenses.export', array_filter($filters))">
        ...
        <form method="GET" action="{{ route('admin.finance.expenses.index') }}" class="rounded-xl border border-[#E6EBF4] bg-white p-4 sm:p-5">
            <div class="grid gap-4 md:grid-cols-12 md:items-end">
                ...
                <div class="flex flex-wrap gap-2 md:col-span-4">
                    ...
                    @if ($hasActiveFilters)
                        <x-common.button type="button" variant="secondary" :href="route('admin.finance.expenses.index')">
        ...
        <div class="overflow-hidden rounded-xl border border-[#E6EBF4] bg-white">
            <div class="hidden border-b border-[#E6EBF4] bg-[#E6EBF4]/50 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-[#080D21] md:grid md:grid-cols-12 md:gap-2">
                <div class="md:col-span-2">{{ __('messages.finance_paid_on') }}</div>
                <div class="md:col-span-3">{{ __('messages.finance_expense_tag') }}</div>
                <div class="md:col-span-2">{{ __('messages.finance_payee_name') }}</div>
            ...
            @forelse ($expenses as $expense)
                <div class="border-b border-[#E6EBF4] px-4 py-3 last:border-b-0 md:grid md:grid-cols-12 md:items-center md:gap-2">
                    <div class="mb-1 text-sm text-[#0F141E] md:col-span-2 md:mb-0">{{ $expense['paid_on'] }}</div>
                    <div class="mb-1 md:col-span-3 md:mb-0">
                        <span class="inline-flex rounded-full bg-[#E5989B]/20 px-2.5 py-0.5 text-xs font-medium text-[#080D21]">
                            {{ $expense['expense_tag_label'] }}
                        </span>
                        @if ($expense['notes'])
                            <p class="mt-1 text-xs text-[#0F141E]/50">{{ $expense['notes'] }}</p>
                        @endif
                    </div>
                    <div class="mb-1 text-sm text-[#0F141E] md:col-span-2 md:mb-0">
                        {{ $expense['payee_name'] ?? '—' }}
                        @if ($expense['salary_breakdown'])
                            <p class="mt-0.5 text-xs text-[#0F141E]/50">{{ $expense['salary_breakdown'] }}</p>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-1 md:col-span-1">
                        ...
                        @can('finance_expenses.delete')
                            <x-common.delete-form
                                :action="route('admin.finance.expenses.destroy')"
                                :message="__('messages.finance_expense_delete_confirm')"
                            >
                                <input type="hidden" name="expense_id" value="{{ $expense['id'] }}">
            ...
            @empty
                <div class="px-4 py-10 text-center text-sm text-[#0F141E]/60">
                    {{ __('messages.finance_expenses_empty') }}
                </div>
            @endforelse
        </div>
    ...
    @include('admin.finance.expenses._edit-modal', [
        'expenseTags' => $expenseTags,
        'workersGrouped' => $workersGrouped ?? [],
        'workerSalaryLookupUrl' => $workerSalaryLookupUrl ?? '',
        'editingExpense' => $editingExpense ?? null,
        'openOnLoad' => $openEditExpenseModal ?? false,
    ])
</x-layouts.admin>
